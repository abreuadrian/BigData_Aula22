...
try:
    ...
    #Intepretação:
    #Quanto maior a variância, maior é o afastamento dos valors em relação a média
    #Neste cados, a variância elevada indica alta dispersão.
...
try:
    ...
    #Rótulo de dados
    deslocamento = max(df_roubo_veiculo_maiores['roubo_veiculo']) * 0.005

    for i, valor in enumerate(df_roubo_veiculo_maiores['roubo_veiculo']):
        plt.text(valor + deslocamento, i, f'{valor:,}', ha='left', fontsize=8)
    ...
    plt.hist(array_roubo_veiculo, bins=10)
    ...
    contagens, limites = np.histogram(array_roubo_veiculo, bins=10) #intervalos próximos a 1.000
    ...
    plt.text(0.5, 0.6, f'Distância da Variância: {distancia_var_media}', fontsize=10)
